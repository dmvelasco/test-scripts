...
"$dir1"/samtools mpileup -BRug -t DP,SP -f "$dir3"/Prunus_persica_v1.0_scaffolds.fa -b "$dir2"/all.amygdalusbam.txt | "$dir1"/bcftools call -O z -Avc -f GQ,GP - > amygdalus_1.raw.vcf.bzip
"$dir1"/bcftools view -O z -o amygdalus_m.flt.vcf.bzip -i "%QUAL>=30 && %AVG(DP)>3 && %AVG(DP)<=100 && %MIN(GQ)>=30" -v snps amygdalus_1.raw.vcf.bzip
