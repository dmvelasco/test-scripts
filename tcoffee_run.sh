...
t_coffee "$pep_dir"/"${gene[0]}"_cds_pep.fasta -method muscle_msa, probcons_msa, dialigntx_msa, clustalo_msa, pcma_msa, mafft_msa, t_coffee_msa -multi_core=4 -output fasta_aln

#### STEP 4: BACK TRANSLATE MULTI-SEQUENCE PEPTIDE FASTA WITH ORIGINAL MULTI-SEQUENCE DNA FASTA
t_coffee -other_pg seq_reformat -in "$dna_dir"/"${gene[0]}"_cds.fa -in2 "$pep_aln_dir"/"${gene[0]}"_cds_pep.fasta_aln -action +thread_dna_on_prot_aln -output fasta_aln > "$dna_aln_dir"/"${gene[0]}"_cds.aln
