...
taxa="PP"						# file prefix
bams="angsd.persicatajdtest.txt"			# file of BAM files for taxa
...
# genotype likelihoods (NEW - WORKS)
#"$dir1"/angsd -bam "$dir2"/"$bams" -anc "$dir4"/"$anc" -ref "$dir3"/"$ref" -nInd -doSaf 1 -GL 2 -P "$threads" -minMapQ "$minMapQ" -minQ "$minQ" -out "$taxa"_tajd2.sfs

#ORIGINAL
"$dir1"/angsd -bam "$dir2"/"$bams" -anc "$dir4"/"$anc" -ref "$dir3"/"$ref" -nInd "$N_IND" -doMajorMinor 1 -doMaf 1 -doSaf 2 -GL 1 -doPost 1 -doGeno 32 -indF "$taxa"tajd_r.indF -r "$r" -nThreads "$threads" -minMapQ "$minMapQ" -minQ "$minQ" -out "$taxa"_"$x"_tajd
...
"$dir5"/realSFS "$taxa"_"$x"_tajd.saf.idx -P "$threads" -r "$r" -maxIter 1000 > "$taxa"_"$x"_tajd.sfs

# ORIGINAL
#"$dir5"/realSFS "$taxa"_tajd.saf.idx "$CHROM" -maxIter 100 -P "$threads" > "$taxa"_tajd.sfs
...
# calculating thetas (NEW - not working...)
#"$dir1"/angsd -bam "$dir2"/"$bams" -anc "$dir4"/"$anc" -ref "$dir3"/"$ref" -doThetas 1 -doSaf 1 -pest "$taxa"_tajd.sfs -GL 2 -P "$threads" -minMapQ "$minMapQ" -minQ "$minQ" -out "$taxa"_tajd.sfs

# ORIGINAL
"$dir1"/angsd -bam "$dir2"/"$bams" -anc "$dir4"/"$anc" -ref "$dir3"/"$ref" -doThetas 1 -doSaf 1 -pest "$taxa"_"$x"_tajd.sfs -GL 2 -P "$threads" -r "$r" -minMapQ "$minMapQ" -minQ "$minQ" -out "$taxa"_"$x"_tajd.sfs
...
zcat "$taxa"_"$x"_tajd.sfs.thetas.gz | perl -plne 's/scaffold_(\w+)/$1/' - > "$taxa"_"$x"_tajdmod.sfs.thetas.gz
# create a binary version of the .thetas.gz
"$dir5"/thetaStat make_bed "$taxa"_"$x"_tajdmod.sfs.thetas.gz
# calculate Tajimas D
"$dir5"/thetaStat do_stat "$taxa"_"$x"_tajdmod.sfs.thetas.gz -nChr "$CHROM" -r "$x" -win 1000 -step 50 -outnames "$taxa"_"$x"_tajdmod.thetasW1kS50.gz
...
echo "$N_SITES"
"$dir6"/ngsStat -npop 1 -postfiles "$taxa"_"$x"_tajd.saf -nsites "$N_SITES" -iswin 1 -nind "$N_IND" -outfile "$taxa"_"$x"_tajd.stat -isfold 0 -islog 1 -block_size "$N_SITES"
