...
"$dir5"/realSFS "$taxa1".saf.idx "$taxa2".saf.idx -P "$threads" > "$taxa3".ml
date
...
"$dir5"/realSFS fst index "$taxa1".saf.idx "$taxa2".saf.idx -sfs "$taxa3".ml -fstout "$taxa3"

# Get the global FST estimate
# Produces unweighted and weighted FST
...
"$dir5"/realSFS fst stats2 "$taxa3".fst.idx -win 1000 -step 200 > "$taxa3"_windows
